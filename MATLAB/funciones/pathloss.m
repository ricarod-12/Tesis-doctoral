function [H_t, H_d_r, H_d_ra, Losses_LOS, Losses_suelo, Losses_arbol, Losses_wide_band_d_r, Losses_wide_band_d_ra, Losses_wide_band_t] = pathloss(f_0,BW,N,dist,perm_real_suelo,a,t,p,teta,hrx,drx_der,drx_izq,htx,dtx_der,dtx_izq,Gto,Gro,offset,polarizacion,mg)
...
for d = 1:length(dist)      % Recorremos todos los valores de distancia
    for f = 1:N             % Recorremos todos los valores del ancho de banda
        ...
        k              = (2*pi)/lambda;
        s              = 0.1;
        perm_img_suelo = 60*s*lambda;       %"Measurements and Characterization at 2.1 GHz While Entering in a Small Tunnel"???
...
%         perm_leaf = e_hoja_real - 1i*e_hoja_img;
        perm_leaf = e_hoja(frecuencias(f),mg);
        ...
        R_hojas_Horizontal_der = coeff_refl_anisotropo(0,alpha_hojas_der,e_t,e_z);          %0 horizontal
        R_hojas_Horizontal_izq = coeff_refl_anisotropo(0,alpha_hojas_izq,e_t,e_z);
        ...
        %Funciones perdidas de cada modelo
        H_LOS(d,f)    = (lambda/(4*pi))*sum(Losses_directas);
        H2_LOS(d,f)   = abs(H_LOS(d,f)).^2;
        
        H_suelo(d,f)    = (lambda/(4*pi))*sum(Losses_reflejadas);
        H2_suelo(d,f)   = abs(H_suelo(d,f)).^2;
        
        H_arboles(d,f)    = (lambda/(4*pi))*sum(Losses_reflejadas_Horizontal_hojas_izq);
        H2_arboles(d,f)   = abs(H_arboles(d,f)).^2;
    end
end
...
for d_2 = 1:length(dist)
    ...
    Losses_LOS(d_2)=-10*log10(sum(H2_LOS(d_2,:))/N)-Gto-Gro;
    Losses_suelo(d_2)=-10*log10(sum(H2_suelo(d_2,:))/N)-Gto-Gro;
    Losses_arbol(d_2)=-10*log10(sum(H2_arboles(d_2,:))/N)-Gto-Gro;
